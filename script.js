...
function renderCycleDetail(type, data, chart, currentYear, currentAge) {
  ...
  if (type === "luck") {
    ...
    detail.innerHTML = renderDetailHtml(`已选大运：${data.ganZhi}（${decadeYears}，${data.startAge}-${data.endAge}岁）`, [
  ...
  if (type === "year") {
    ...
    detail.innerHTML = renderDetailHtml(`已选流年：${data.year}年 ${pillar}`, [
  ...
  if (type === "month") {
    ...
    detail.innerHTML = renderDetailHtml(`已选流月：${data.label} ${data.pillar}`, [
    ...
    document.querySelector("#luck-cycle").textContent = `已选流月：${data.pillar}`;
  }
}
...
function renderFlowMonths(year, chart, activeLuck, yearPillar, currentAge, autoRender = true) {
  ...
  monthTrack.innerHTML = months.map((month, index) => {
    ...
    return `<button class="cycle-card${selected}" type="button" data-cycle="month" data-year="${year}" data-month="${index}"><span>${month.label}</span><strong>${month.pillar}</strong><em>${tenGod(dayStem, month.stem)}</em></button>`;
  }).join("");
